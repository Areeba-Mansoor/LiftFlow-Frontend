...
//             We'll generate a secure link to reset your password.
//           </p>
//         </div>
...
//           {resetLink && (
//             <div className="mb-5 px-4 py-3 bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs rounded-lg font-medium space-y-2">
//               <p>Reset link generated (demo mode — no email sent):</p>
//               <Link to={resetLink.replace(/^https?:\/\/[^/]+/, '')} className="underline break-all font-semibold">
//                 {resetLink}
//               </Link>
//             </div>
//           )}
...
//                   className="w-full bg-white border border-slate-200 rounded-lg pl-9 pr-4 py-2.5 text-sm outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/10 transition-all"
//                 />
//               </div>
//             </div>
...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Agar aap user ko automatic OTP verification page par bhejna chahte hain
      setTimeout(() => {
        navigate('/reset-password'); // Ya jo bhi apka OTP/reset route ho
      }, 1500);
    } catch (err) {
      setError(err.response?.data?.message || 'Something went wrong.');
    } finally {
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => setEmail(e.target.value)}
                  placeholder="name@company.com"
                  className="w-full bg-white border border-slate-200 rounded-lg pl-9 pr-4 py-2.5 text-sm outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/10 transition-all text-slate-800"
                />
              </div>
            </div>
