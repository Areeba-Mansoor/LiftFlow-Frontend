...
// import { Lock, MoveVertical, Loader2, Eye, EyeOff } from 'lucide-react';
...
import { useNavigate, useParams, Link } from 'react-router-dom';
import API from '../services/api';
import { Lock, MoveVertical, Loader2, Eye, EyeOff, KeyRound } from 'lucide-react';

export default function ResetPassword() {
  const { token } = useParams(); // Agar route me token/email ki zaroorat ho to use kar sakte hain
  const navigate = useNavigate();
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setLoading(true);
    try {
      // Apne backend endpoint ke mutabiq URL adjust karlein (e.g., /auth/verify-otp)
      await API.post('/auth/verify-otp', { otp });
      setSuccess('OTP verified successfully.');
      setTimeout(() => {
        setSuccess('');
        setStep(2); // Agle step par move karein jahan naya password set hoga
      }, 1000);
    } catch (err) {
    ...
    <div className="min-h-screen w-full flex flex-col lg:flex-row bg-[#F8FAFC] font-['Inter']">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 lg:py-16">
        <div className="w-full max-w-sm">
          ...
          <h1 className="font-['Space_Grotesk'] text-2xl font-semibold text-slate-900 mt-2">
            {step === 1 ? 'Enter OTP code' : 'Reset password'}
          </h1>
          <p className="text-sm text-slate-500 mt-1 mb-8">
            {step === 1
          ...
          {success && (
            <div className="mb-5 px-4 py-2.5 bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs rounded-lg font-medium">
              {success} {step === 2 && 'Redirecting to login...'}
            </div>
          )}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="123456"
                    className="w-full bg-white border border-slate-200 rounded-lg pl-9 pr-4 py-2.5 text-sm tracking-widest text-slate-800 outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/10 transition-all font-mono"
                  />
                </div>
              </div>
